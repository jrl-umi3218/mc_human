...
HumanRobotModule::HumanRobotModule(bool fixed, bool canonical)
  ...
  if (!canonical) {
    for (const auto& link :
         {"HandThumb", "HandIndex", "HandMiddle", "HandRing", "HandBaby"})
  ...
  // Accelerometer
  _bodySensors.emplace_back("Accelerometer", "HipsLink",
                            sva::PTransformd(Eigen::Vector3d(0, 0, 0.1095)));
  // // Floating base
  _bodySensors.emplace_back("FloatingBase", "HipsLink",
...
std::map<std::string, std::vector<double>> HumanRobotModule::halfSittingPose(
  ...
  for (const auto& j : mb.joints()) {
    if (halfSitting.count(j.name())) {
      ...
      mc_rtc::log::warning(
          "Joint {} has {} dof, but is not part of half sitting posture.",
          j.name(), j.dof());
    }
  }
  return res;
...
HumanRobotModule::getConvexHull(
  ...
  for (const auto& f : files) {
    ...
    if (fs::exists(fpath)) {
      res[f.first] = std::pair<std::string, std::string>(
          f.second.first, convexPath + f.second.second + "-ch.txt");
    }
  }
  return res;
}
...
std::map<std::string, std::pair<std::string, std::string>>
HumanRobotModule::stdCollisionsFiles(const rbd::MultiBody& /*mb*/) const {
  std::map<std::string, std::pair<std::string, std::string>> res;
  for (const auto& b : mb.bodies()) {
    // Filter out virtual links without convex files
    if (std::find(std::begin(virtualLinks), std::end(virtualLinks), b.name()) ==
        std::end(virtualLinks)) {
